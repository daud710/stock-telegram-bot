"""
🤖 Indian Stock Market Telegram Bot — UPGRADED
================================================
NEW FEATURES:
...
# ─────────────────────────────────────────────
def make_ascii_chart(prices, width=20, height=8):
    """Generate a beautiful ASCII price chart."""
    ...
    for row in range(height):
        ...
        for i, p in enumerate(prices[-width:]):
            if p >= threshold - rng / (height * 2):
                if i == len(prices[-width:]) - 1:
                    line += "●"
                elif prices[-width:][i] > prices[-width:][i-1]:
                    line += "▲"
                else:
                    line += "▼"
            else:
...
def make_bar_chart(prices, width=24):
    """Simpler bar-style chart using blocks."""
    if not prices or len(prices) < 2:
        return "No data"
...
def get_stock_history(symbol, period="3mo"):
    """Fetch complete stock history and analysis."""
    ...
    try:
        ...
        # Support & Resistance (simple: 1-month low/high)
...
# ─────────────────────────────────────────────
# 💬 FORMAT STOCK HISTORY MESSAGE
# ─────────────────────────────────────────────
def format_stock_detail(d):
    """Format beautiful stock detail message."""
    # Signal emoji
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = (
        "🤖 *Indian Stock Market Bot — UPGRADED!*\n\n"
...
async def send_stock_detail(update_or_message, symbol):
    """Core function to fetch and send stock detail."""
    msg_obj = update_or_message if hasattr(update_or_message, 'reply_text') else update_or_message.message
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Handle direct stock name messages."""
    ...
    if text in NSE_STOCKS or (len(text) >= 2 and len(text) <= 15 and text.isalpha()):
        ...
        await update.message.reply_text(
            "Stock ka naam bhejo jaise:\n`RELIANCE` ya `TCS` ya `INFY`\n\nYa /help type karo sab commands ke liye.",
            parse_mode=ParseMode.MARKDOWN
        )

async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Handle inline button clicks for period selection."""
    ...
    await query.edit_message_text(
        f"🔄 _{symbol} — {period} data load ho raha hai..._",
        parse_mode=ParseMode.MARKDOWN
    )
    try:
